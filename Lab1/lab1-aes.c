...
void addRoundKey(int array[4][4], int round)
{
  ...
  int temp;
  for(int i=0;i<4;i++){
    splitIntToArray(w[i+round*4],keys[i]);
  }
  for(int i=0;i<4;i++){
    for(int j=i;j<4;j++){
        temp = keys[i][j];
...
//请补充代码
    if(num%4==0){
        int temp;
        temp = ((w[num-1]&0x00ffffff)<<8)|((w[num-1]>>24)&0x000000ff);
        int byte1 = getNumFromSBox((temp>>24)&0x000000ff);
        byte1 = byte1<<24;
        int byte2 = getNumFromSBox((temp>>16)&0x000000ff);
        byte2 = byte2<<16;
        int byte3 = getNumFromSBox((temp>>8)&0x000000ff);
        byte3 = byte3<<8;
        int byte4 = getNumFromSBox(temp&0x000000ff);
        temp = byte1|byte2|byte3|byte4;
        return w[num-4]^(temp^Rcon[round-1]);
    }
    else{
        return w[num-4]^w[num-1];
    }
